# Software/DataHarvester/services/data_transformation/application/services/prompt_loader.py
...
class PromptManager:
    ...
    def get_prompt(template: str, **kwargs) -> str:
        ...
        if not env or not env.loader:
            raise ValueError("Template environment or loader not properly initialized")
            
        template_path = f"{template}.j2"
        template_source = env.loader.get_source(env, template_path)
        if template_source:
            with open(str(template_source[1]), 'r') as file:
                post = frontmatter.load(file)

        template_str = env.get_template(template_path).render(**kwargs)
        try:
            return template_str
    ...
    def get_template_info(template: str) -> dict:
        ...
        env = PromptManager._get_env()
        template_path = f"{template}.j2"
        template_source = env.loader.get_source(env, template_path) # type: ignore
        if template_source:
            with open(str(template_source[1]), 'r') as file:
                post = frontmatter.load(file)
